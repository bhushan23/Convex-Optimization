...
###############################################################################
def backtracking( func, x, direction, alpha=0.4, beta=0.9, maximum_iterations=1000): #65536 ):
    """ 
...
###############################################################################
def newton( func, initial_x, eps=1e-4, maximum_iterations=65536, linesearch=bisection, *linesearch_args  ):
    """ 
    Newton's Method
...
def objective_scalar_constraints( constraints, x, order=0 ):
    ...
    for constraint in constraints:
        ...
        constraint_value = np.asscalar(constraint_value)
        if order >= 1:
            ...
            gradientVal = -1 * (constraint_gradient / constraint_value)
            gradients.append(gradientVal)
            ...
            if order == 2:
                ...
                hessianVal =  (constraint_gradient*constraint_gradient.T)/ (constraint_value**2) - (constraint_hessian / constraint_value)
                hessians.append(hessianVal)
